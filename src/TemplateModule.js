...
export function Main(props) {
  ...
  const bufferToDigest = () => {
    ...
    let didResolutionMedaDataJSON = {
      DIDResolutionMedatadataKey1: "DIDResolutionMedatadataValue1",
    };

    setDIDDocumentMetaData(
      Array.from(new Uint8Array(str2ab(JSON.stringify(didMetaDataJSON))))
    );
    setdidResolutionMetadata(
      Array.from(
        new Uint8Array(str2ab(JSON.stringify(didResolutionMedaDataJSON)))
      )
    );
